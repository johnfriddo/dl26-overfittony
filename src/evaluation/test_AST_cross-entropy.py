...
CHECKPOINT = './experiments/checkpoints/best_ast_baseline.pth'
NUM_CLASSES = 44
BATCH_SIZE = 32
...
def evaluate():
    ...
    model = EPICASTBaseline(num_classes=NUM_CLASSES).to(device)
    
    if not os.path.exists(CHECKPOINT):
        print(f"\n[ERRORE] Checkpoint non trovato: {CHECKPOINT}")
        print("Assicurati di aver addestrato il modello e che il path sia corretto.")
        return
    ...
    for inputs, labels in tqdm(loader, desc="Test AST Baseline"):
    ...
    print("\n" + "=" * 45)
    print(" RISULTATI FINALI SUL TEST SET (AUDIO-ONLY) ")
    print("=" * 45)
    ...
    df_results = pd.DataFrame({
        'modello': ['AST_CrossEntropy'],
        'top1_acc': [top1],
        'top5_acc': [top5],
        'mAP': [map_score],
        'mCA': [mca]
    })
    df_results.to_csv('risultati_test_ast.csv', index=False)
    print("\nRisultati salvati in: risultati_test_ast.csv")
