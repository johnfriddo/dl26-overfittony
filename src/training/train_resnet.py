...
# ==========================================
# 0. SCELTA DELL'ESPERIMENTO
# ==========================================
FUSION = 'late_pool'
NUM_FRAMES = 1
RUN = f"{FUSION}-T{NUM_FRAMES}"
...
PESI_OFFLINE = 'resnet50_pesi.pth'
...
wandb.init(
    ...
    config={
        ...
        "dropout": 0.5,
        "label_smoothing": 0.1,
        "architettura": "ResNet50",
    },
)
cfg = wandb.config

# ==========================================
# 2. TRASFORMAZIONI E DATALOADER
# ==========================================
...
print(f"Esperimento: fusion={FUSION}, T={NUM_FRAMES} | device: {device}")

# ==========================================
# 3. MODELLO, LOSS, OTTIMIZZATORE
# ==========================================
modello = EpicResNet(num_verbs=97, num_nouns=300, weights=PESI_OFFLINE,
                     dropout=cfg.dropout, fusion=cfg.fusion,
                     num_frames=cfg.num_frames).to(device)
...
# raggruppamento generico: tutto cio' che NON e' backbone (teste + eventuale MLP di
# fusione) va nel gruppo "head" con LR piu' alto -> cosi' i parametri di 'fuse'
# vengono ottimizzati anche in late_fc
...
# cosine: usa tutto il budget di LR senza i tagli bruschi del ReduceLROnPlateau
...
for epoch in range(cfg.epochs):
    ...
    scheduler.step()   # cosine: un passo per epoca

    # selezione/stop su METRICA (media verb+noun top-1), non sulla val loss
