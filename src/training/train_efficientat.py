...
def main():
    ...
    wandb.init(
        project="epic-sounds-efficientat",
    ...
    # 1. lettura delle annotazioni
    full_df = pd.read_csv(args.annotations_file)
    
    # 2. separare i video (80% dei VIDEO in train, 20% in val)
    ...
    # 3. salva due CSV temporanei per istanziare i dataset
    ...
    print("Caricamento dataset...")
    # 4. Inizializza i dataset separatamente
    ...
    model = EPICEfficientATBaseline(num_classes=args.num_classes)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        try:
            ...
            mauc_score = 0.0 # Fallback sicuro
        ...
        if map_score > best_map:
            best_map = map_score
            patience_counter = 0
            checkpoint_path = os.path.join(args.checkpoint_dir, "best_efficientat.pth")
