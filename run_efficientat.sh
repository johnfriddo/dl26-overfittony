#!/bin/bash
#SBATCH --job-name=eff_baseline
#SBATCH --output=experiments/logs/training_eff_%j.out
#SBATCH --error=experiments/logs/training_eff_%j.err
...
if [ ! -d "metadata" ]; then
    ln -s /home/rsnnng02c19b202w/dl26-overfittony/src/models/EfficientAT/metadata metadata
fi
...
singularity exec --nv /shared/sifs/latest.sif python -m src.training.train_efficientat \
